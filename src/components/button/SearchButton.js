...
const Button = () => {
  ...
  let url = `/destinations/hotels?query=${destination}&date_from=${format(new Date(dateSearch[0].startDate), "dd-MM-yyyy")}&date_to=${format(new Date(dateSearch[0].endDate), "dd-MM-yyyy")}`;
  // if (destination !== "") {
  ...
  return (
    <button
      ...
      className={`${destination === "" ? "bg-red-900 disabled:opacity-70" : "bg-red-900"} py-4 px-9 uppercase text-white text-xs font-semibold rounded-[3px] cursor-pointer`}
    >
      search
    </button>
  );
